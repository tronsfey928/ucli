/**
 * End-to-end integration tests: StreamableHTTP transport
 *
 * Starts a real Express + StreamableHTTP MCP server on a random port,
...
beforeAll((done) => {
  ...
  // Session map: sessionId → transport (stateful mode)
  ...
  async function handleMcp(req: express.Request, res: express.Response): Promise<void> {
    ...
    if (!transport) {
      if (sessionId) {
        // Known session ID but transport not found (expired or closed)
    ...
    // handleRequest processes the request (and for initialize, assigns the session ID).
    // Store in the map afterwards so the session ID is available for subsequent requests.
...
describe('tool listing (StreamableHTTP)', () => {
...
describe('tool execution — pretty output (StreamableHTTP)', () => {
...
describe('tool execution — raw output (StreamableHTTP)', () => {
...
describe('tool execution — JMESPath filtering (StreamableHTTP)', () => {
...
describe('error handling (StreamableHTTP)', () => {
...
describe('bake (StreamableHTTP)', () => {
  const bakeName = `e2e-streamable-${Date.now()}`;
